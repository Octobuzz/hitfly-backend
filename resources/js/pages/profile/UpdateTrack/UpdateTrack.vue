<template>
  <div class="trackInfo">
    <ReturnHeader />
    <div v-if="isLoading" class="add-track-loading">
      <SpinnerLoader />
    </div>
    <div v-else class="add-track">
      <div class="add-track-cover">
        <ChooseAvatar
          :image-url="track.cover[0].url"
      ...
      <div class="add-track-description">
        <BaseInput
          ...
          :show-error="validation.trackName.error"
          :error-message="validation.trackName.message"
        >
          <template #icon>
            <PencilIcon />
          </template>
        ...
        <ChooseGenres
          ...
          :none-selected-error="validation.genre.error"
        />

        <h3 class="add-track-description__header_h3">
          Музыкальная группа
        </h3>
        <p class="add-track-description__subheader">
          Выберите автора песни: вы или группа
        </p>
        <BaseDropdown
        ...
        <div v-show="addToAlbum" class="addAlbumWrapper">
          <div class="addAlbumWrapper__buttons">
            <div
              class="buttonSwitcher"
              :class="{active: createAlbum}"
              @click="createAlbum = true"
            >
              Создать альбом
            </div>
            <div
              v-if="albums.albums.data.length > 0"
              class="buttonSwitcher"
              :class="{active: !createAlbum}"
              @click="createAlbum = false"
            >
              Мои альбомы
            </div>
          </div>
          ...
          />
          <div v-show="!createAlbum" class="addAlbumWrapper__content">
...
export default {
  ...
  methods: {
    ...
    addInfo() {
      if (!this.isLoading) {
        const genres = this.trackInfo.genres.map(genre => genre.id);
        let singer = null;
        let band = null;
        let bandId = null;
        if (this.bands[0].name === this.trackInfo.selectedArtist) {
          ...
          band = this.bands.filter(band => band.name === this.trackInfo.selectedArtist);
          bandId = band[0].id;
          singer = band[0].name;
        }
        let info = {
  ...
  apollo: {
    ...
    musicGroups() {
      return {
        ...
        update(data) {
          const myData = { id: 0, name: data.myProfile.username };
